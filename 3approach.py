...
def repair(a):
    ...
    repaired_onco = 0
    non_repaired_onco = 0
    p3 = organizm[a].par_a * organizm[a].passage + organizm[a].par_b
    # print(min(p3, p4), organizm[a].par_a * organizm[a].passage + 1, organizm[a].par_b)
    for i in range(organizm[a].damage):
        ran = random.random()
        if ran < min(p3, p4):  # próba naprawy
            ran = random.random()
            # print(min(p3, p4)*p4)
            if ran < p4:
...
uszk = 3  # liczba uszkodzeń w kroku
