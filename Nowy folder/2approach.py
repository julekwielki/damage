...
def repair(a):
    ...
    if a == 0:
        print(p3, organizm[a].par_a * organizm[a].age + organizm[a].par_b)
    ...
    return repaired, non_repaired, repaired_onco, non_repaired_onco
...
uszk = 10  # liczba uszkodzeń w kroku
    ...
    for x in range(nn):
        if organizm[x].status == 1:  # normalna komórka
            ...
            if rand < p1:
                ...
                rep, non_rep, rep_on, non_rep_on = repair(x)
...
title = "Podejście 2. Uszkodzenia = " + str(uszk)
plt.title(title)
plt.legend()
plt.show()
